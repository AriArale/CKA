...
* **範例：** `Web App` → `Database`。代表 App 主動連線 DB。

### B. 時序圖 (Sequence Diagram)
* **畫法：** **雙向拆解** (實線 Request → / 虛線 Response $\dashleftarrow$)。
...
* **範例：** OAuth 登入流程、K8s Pod 建立流程 (Kubectl → API → Etcd)。需確認每個步驟是否成功 ACK。
...
* **範例：** Prometheus (Pull 模式) 監控 Node Exporter。
    * **架構連線面 (TCP):** `Prometheus` → `Node Exporter` (Prometheus 主動發起連線去抓)。
